...
public class GeometryGrassPainter : MonoBehaviour
{
    ...
    public float CutHere(Vector3 mowerPos, float radius, float remainLength)
    {
        lastNCut = curNCut;
        for (int j = 0; j < positions.Count; j++)
        {
            ...
            float dist = Vector3.Distance(mowerPos, pos);

            // // if its within the radius of the brush, remove all info
            // if (dist <= radius && remainLength <= 0.000001f)
            // {
            ...
            if(dist <= radius)
            {
                if (length[j][1] >= sizeLength - 0.1)
                {
                    curNCut++;
                }
                length[j] = new Vector2(sizeWidth, remainLength);
            }
        }
        ...
        return (float)(curNCut - lastNCut) / (float)positions.Count;
    }
